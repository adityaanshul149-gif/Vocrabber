...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  activeVocab.forEach(v => {
    ...
    if (state !== 'Never Practiced') {
      encounteredCount++;
      totalNetPoints += score;
      if (score < 0) negativeScoreCount++;
      else if (score > 0) positiveScoreCount++;
      else zeroScoreCount++;
    }
    if (state === 'Mastered') {
      masteredCount++;
  ...
  const averagePointsPerWord = encounteredCount > 0 ? (totalNetPoints / encounteredCount).toFixed(1) : '0';

  const getScoreColorBg = (score: number) => {
    if (score > 0) return 'bg-[#4ADE80] text-black';
    if (score < 0) return 'bg-[#FF6B6B] text-black';
    ...
    <div className="space-y-4 font-sans max-w-md mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-[#4ADE80] text-black border-2.5 border-black rounded-2xl p-3.5 shadow-[3.5px_3.5px_0px_0px_#000]">
          <span className="text-black/80 text-[10px] font-black uppercase block">Mastered (≥3 pts)</span>
          <strong className="text-2xl font-black font-display text-black block mt-0.5">
            {masteredCount}
          </strong>
        </div>
        <div className="bg-[#FF6B6B] text-black border-2.5 border-black rounded-2xl p-3.5 shadow-[3.5px_3.5px_0px_0px_#000]">
          <span className="text-black/80 text-[10px] font-black uppercase block">Needs Work (&lt;0 pts)</span>
          <strong className="text-2xl font-black font-display text-black block mt-0.5">
            {negativeScoreCount}
          </strong>
        </div>
        ...
        <div className={`${getScoreColorBg(totalNetPoints)} border-2.5 border-black rounded-2xl p-3.5 shadow-[3.5px_3.5px_0px_0px_#000]`}>
          <span className="text-black/80 text-[10px] font-black uppercase block">Net Points 🏆</span>
          <strong className="text-2xl font-black font-display text-black block mt-0.5">
            {totalNetPoints > 0 ? `+${totalNetPoints}` : totalNetPoints} pts
          </strong>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border-2.5 border-black dark:border-white rounded-2xl p-5 space-y-4 shadow-[4px_4px_0px_0px_#000] dark:shadow-[4px_4px_0px_0px_#A855F7]">
        <div className="flex items-center justify-between border-b-2 border-black dark:border-slate-800 pb-2">
          <h3 className="font-black font-display uppercase text-slate-900 dark:text-white text-sm">Points & Mastering Summary</h3>
          <span className="text-xs font-black text-purple-600 dark:text-purple-400">
            Avg: {averagePointsPerWord} pts/word
          </span>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <div className="flex justify-between text-xs text-black dark:text-slate-300 mb-1 font-bold">
              <span>Mastered Words (≥3 pts)</span>
              <span className="font-black text-slate-900 dark:text-white">
                {masteredCount} / {activeVocab.length}
              </span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-xs text-black dark:text-slate-300 mb-1 font-bold">
              <span>Points Performance breakdown</span>
              <span className="font-black text-slate-900 dark:text-white">
                +{positiveScoreCount} | -{negativeScoreCount}
              </span>
            </div>
            <div className="flex h-3.5 rounded-xl bg-slate-200 dark:bg-slate-800 border-2 border-black dark:border-white overflow-hidden p-0.5 gap-0.5">
              <div
                className="h-full bg-[#4ADE80] border border-black rounded-l-md"
                style={{ width: `${encounteredCount > 0 ? (positiveScoreCount / encounteredCount) * 100 : 0}%` }}
                title={`Positive score words: ${positiveScoreCount}`}
              />
              <div
                className="h-full bg-[#FFE600] border border-black"
                style={{ width: `${encounteredCount > 0 ? (zeroScoreCount / encounteredCount) * 100 : 0}%` }}
                title={`Zero score words: ${zeroScoreCount}`}
              />
              <div
                className="h-full bg-[#FF6B6B] border border-black rounded-r-md"
                style={{ width: `${encounteredCount > 0 ? (negativeScoreCount / encounteredCount) * 100 : 0}%` }}
                title={`Negative score words: ${negativeScoreCount}`}
              />
            </div>
          </div>
        </div>
